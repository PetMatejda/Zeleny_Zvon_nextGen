...
export default function EShopClient({ initialProducts }) {
  ...
  const filteredProducts = activeFilter === 'Všechny produkty' ? products : products.filter(p => p.category === activeFilter);
    ...
    <div className="group flex flex-col justify-between h-full">
      <Link href={`/eshop/${product.slug || product.id}`} className="block">
        <div className="relative overflow-hidden rounded-xl bg-surface-container-low mb-4 aspect-[4/5] cursor-pointer">
          ...
          <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); addToCart(product); }} className="absolute bottom-4 right-4 bg-surface-container-lowest/90 backdrop-blur-md p-3 rounded-full shadow-lg opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 hover:scale-110 cursor-pointer">
      ...
      <div className="px-2 flex-grow flex flex-col justify-end">
        ...
        <div className="flex justify-between items-center mt-auto">
          ...
          <button onClick={(e) => { e.stopPropagation(); addToCart(product); }} className="text-sm font-medium text-[#765a17] dark:text-[#ffdf9f] underline underline-offset-4 hover:text-primary-container transition-colors">Vložit do košíku</button>
        </div>
      </div>
    </div>
  );
      ...
      <section className="mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="flex flex-wrap gap-2">
          {categories.map(filter => (
            <button key={filter} onClick={() => setActiveFilter(filter)} className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${activeFilter === filter ? 'bg-primary-container text-on-primary' : 'bg-surface-container-high text-on-surface-variant hover:bg-[#765a17] hover:text-white'}`}>
              {filter}
            </button>
          ))}
        </div>
