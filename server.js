...
app.post('/api/contact', async (req, res) => {
  ...
  if (!name || !email || !message) {
    return res.status(400).json({ error: 'Uveďte jméno, e-mail a zprávu.' });
  }
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      to: process.env.ADMIN_EMAIL || 'zelenyzvon@gmail.com', // Poslat majiteli na stejnou nebo administrátorskou adresu
      replyTo: email,
    ...
    if (transporter.options.host === 'smtp.ethereal.email') {
      console.log("Náhled testovací kontaktní zprávy: %s", nodemailer.getTestMessageUrl(info));
    } else {
      console.log("Kontaktní zpráva odeslána na zelenyzvon@gmail.com");
    }

    res.json({ success: true });
  } catch (err) {
    console.error("Nepodařilo se odeslat kontaktní e-mail: ", err);
    res.status(500).json({ error: 'Zprávu se nepodařilo odeslat. Zkuste to prosím později.' });
  }
});
